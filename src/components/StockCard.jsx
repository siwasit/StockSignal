...
function StockCard({ logo, stockSymbol, status, reason, timeStamp, isFavorite, onToggleFavorite, onClick, price }) {
    ...
    function StatusIcon({ status }) {
        if (status === 'Buy') return <div className="w-0 h-0 border-l-8 border-r-8 border-b-10 border-transparent border-b-[#41DC8E]" />;
        if (status === 'Hold') return <div className="w-4 h-4 rounded-full bg-[#E0B469]" />;
        if (status === 'Sell') return <div className="w-0 h-0 border-l-8 border-r-8 border-t-10 border-transparent border-t-[#FF8282]" />;
        return null;
    }
